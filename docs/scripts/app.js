(() => {
  $(function () {
    ...
    $('.special-character').on('click', function (event) {
      ...
      if ($lastActive) {
        // This Boolean checks to see if the last active input
    ...
    $('#submit').on('click', function (event) {
      ...
      $('.question').each(function () {
        ...
        if (wasCorrect) {
          //Adds CSS to show green/red indicator around the input box.
        ...
        if ($commonMistake != null) {
          $commonMistake.removeClass('hidden').parent().removeClass('hidden');
        } else {
    ...
    $('#questionnaire-button').on('click', function (event) {
      ...
      $('small').replaceWith(
        '<small>In the textboxes below, fill out the fully declined version of the word in brackets.</small>',
      );
      $('#table-submit').replaceWith(
        '<button type="submit" class="solid-button button" id="submit">Check</button>',
      );
      $('#table-try-again').replaceWith(
        '<button type="button" class="solid-button button hidden" id="again">Try Again?</button>',
      );
    });
  });

  function loadQuestionsInto($container, filename) {
    // Now container=questionWrapper (where the questions are stored on the page)
    // exit if we don't have all params
    if (!$container || !filename) {
      return;
    }

    $.getJSON(`${window.BASE_PATH}data/${filename}?cache=` + Date.now()) // This is a slight modification to cause 'cache-busting'. Basically, it appends a number to the url to prevent the browser caching this file.
      ...
      .done(function (data) {
        ...
        selectedQuestionsArray.forEach(function (question) {
          //What follows is what creates the template for our questions.
  ...
  function flashcardCreate($mContainer, fFilename) {
    ...
    $.getJSON(`${window.BASE_PATH}data/${fFilename}?cache=` + Date.now()).done(function (data) {
  ...
  $('.flashcard-answer').on('keydown', function (event) {
    ...
    if (event.keyCode === 13 && checkButton === false) {
      //This makes sure the Check button is visible so you can't trigger the check multiple times using the Enter key
      event.preventDefault();
  ...
  $('.translation-button').on('click', function () {
    ...
    if (dataLanguage == 'old') {
      ...
      $('.translation-button').html(
        'Modern English <i class="fas fa-exchange-alt"></i> Old English',
      );
      $('.flashcard').empty().removeClass('correct-flashcard incorrect-flashcard');
      ...
      $('.translation-button').html(
        'Old English <i class="fas fa-exchange-alt"></i> Modern English',
      );
      $('.flashcard').empty().removeClass('correct-flashcard incorrect-flashcard');
  ...
  $('.reader-word').on('click', function (event) {
    ...
    if ($('.show-old-english').hasClass('solid-button')) {
      ...
      if (typeof $(this).data('base-form') !== 'undefined') {
        $('.reader-tooltip-body')
          .find('.reader-tooltip-p:first-child')
          .after('<p>Base form: <em>' + baseForm + '</em></p>');
      }
    }
  });
  ...
  $('.show-notes').on('click', function (event) {
    ...
    if ($('.show-notes').hasClass('light-button')) {
      ...
      $('.footnote').each(function (index) {
        $(this).after(
          `<a class="footnote-link" id="footnoteref${index + 1}" href="${window.location.pathname}#footnote${index + 1}"><sup class="darkorange-text">[${index + 1}]</sup></a>`,
        );
      });
      $('.scrollable-area')
        .find('p')
        .each(function (index) {
          $(this).prepend(
            `<a href="${window.location.pathname}#footnoteref${index + 1}" class="darkorange-text"><strong id="footnote${index + 1}">[${index + 1}]</strong></a> `,
          );
        });
    } else {
